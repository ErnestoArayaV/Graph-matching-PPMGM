...
for k = 1:MAX_ITER
      ...
      %  x = R \ (R' \ (rho*(z - u) - q));
       ...
       %U=reshape(u,[n,n]);
        ...
        vec_2=lambda.*vec_1(:);
        %auxmat=reshape(vec_2,[n,n]);
        auxmat=reshape(vec_2.', n, []).';
        %auxmat-reshape(vec_2,[n,n])'
        %mat_1=V_A*vec2mat(vec_2,n)'*V_B';
        mat_1=V_A*auxmat'*V_B';
        ...
        % compute - (H+I)^-1 (A' mu)
        %auxmat2=reshape(vec3,[n,n]);
         auxmat2=reshape(vec3.', n, []).';
        %vec_4=V_A'*vec2mat(vec3,n)'*V_B;
        vec_4=V_A'*auxmat2'*V_B;
        vec_5=lambda.*vec_4(:);
        auxmat3=reshape(vec_5.', n, []).';
       % auxmat3=reshape(vec_5,[n,n]);
        %mat_2=V_A*vec2mat(vec_5,n)'*V_B';
        mat_2=V_A*auxmat3'*V_B';
